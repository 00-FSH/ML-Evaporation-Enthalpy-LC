...
# 定义用于多个随机种子评估的函数
def evaluate_with_multiple_seeds(n_estimators, max_depth, min_samples_split, min_samples_leaf, data, targets):
    results = []
    ...
    for seed in seeds:
        train_r2, test_r2, train_rmse, test_rmse, train_mae, test_mae = evaluate_rf(
            n_estimators, max_depth, min_samples_split, min_samples_leaf, data, targets, random_seed=seed
        )
        results.append({
            "Random Seed": seed,
            "Train R²": train_r2,
    ...
    file_name = "D:\pycharm\机器学习\pythonProject\Master3\ML\SMILE\随机种子\ExtraTrees_all_seed_performance_metrics_0.2.xlsx"
    ...
    evaluate_rf(n_estimators, max_depth, min_samples_split, min_samples_leaf, data, targets, random_seed=290,
                save_predictions=True)


def evaluate_rf(n_estimators, max_depth, min_samples_split, min_samples_leaf, data, targets, random_seed=42,
    ...
    # Train the ExtraTreesRegressor with the best parameters
...
def save_predictions_to_excel(y_train, y_pred_train, y_test, y_pred_test, random_seed):
    ...
    # Save to Excel
    file_name = f"D:\pycharm\机器学习\pythonProject\Master3\ML\SMILE\随机种子\ExtraTrees_predictions_seed_290_0.2.xlsx"
...
def optimize_rf(data, targets):
    """Apply Bayesian Optimization to ExtraTrees parameters."""

    def rfc_single_seed(n_estimators, max_depth, min_samples_split, min_samples_leaf):
        return evaluate_rf(
            n_estimators=int(n_estimators),
            max_depth=int(max_depth),
            min_samples_split=int(min_samples_split),
    ...
    logger = JSONLogger(path="D:\pycharm\机器学习\pythonProject\Master3\ML\SMILE\随机种子\ExtraTrees_rank12_test.json")
    optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)
    start = time.time()
    optimizer.maximize(init_points=5, n_iter=55)
    with open('D:\pycharm\机器学习\pythonProject\Master3\ML\json_smile\ExtraTrees_rank12_test.json', 'a', encoding='utf-8', errors='replace') as f:
    ...
    evaluate_with_multiple_seeds(n_estimators, max_depth, min_samples_split, min_samples_leaf, data, targets)
